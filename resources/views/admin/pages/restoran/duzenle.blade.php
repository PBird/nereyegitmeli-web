 ...
 @include('admin.layouts.messages')
   ...
   <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>#{{$sehir->isim}} - {{$restoran->isim}}</h1>
          </div>
          <div class="col-sm-6">
            ...
            <div class="card-body">
              ...
              <div class="card card-primary">
                  <div class="card-header">
                    <h3 class="card-title">{{$restoran->isim}}</h3>
                  </div>
                  <!-- /.card-header -->
                  <!-- form start -->
                  <form role="form" id="olusturForm" action="{{route('restoran.update',$restoran->id)}}" method="POST" enctype="multipart/form-data">
                  {{method_field('put')}}
                  {{csrf_field()}}
                   <input type="hidden" id="markerAddress" name="markerAddress" value="@if(old('markerAddress')) {{old('markerAddress')}} @else {{$restoran->markerAddress}} @endif">
                   <input type="hidden" id="position" name="position" value="@if(old('position')) {{old('position')}} @else {{$restoran->position}} @endif">
                  <input type="hidden" name="sehir" value="{{$sehir->id}}">
                    <div class="card-body">
                      <div class="form-group">
                        <label for="isim">İsim</label>
                        <input type="text" class="form-control" id="isim" name="isim" placeholder="Restoran ismini giriniz" value="@if(old('isim')) {{old('isim')}} @else {{$restoran->isim}} @endif">
                      </div>
                      <div class="form-group">
                          <label>Adres</label>
                          <textarea class="form-control" rows="3" placeholder="Adresi giriniz" name="adres">@if(old('adres')) {{old('adres')}} @else {{$restoran->adres}} @endif</textarea>
                      </div>
                      ...
                      <div class="input-group">
                        ...
                        <input type="text" class="form-control bfh-phone" data-format="+0 (ddd) ddd-dddd"  name="telefon" value="@if(old('phone')) {{old('phone')}} @else {{$restoran->phone}} @endif">
                      </div>

                      <div class="form-group">
                        <label for="aciklama">Kısa Açıklama</label>
                        <input type="text" class="form-control" id="aciklama" name="kisaaciklama" placeholder="Kısa açıklama" value="@if(old('kisaaciklama')) {{old('kisaaciklama')}} @else {{$restoran->kisaaciklama}} @endif">
                      </div>
                       <div class="form-group">
                        <label for="aciklama">Uzun Açıklama</label>
                        <textarea class="form-control" rows="3" placeholder="Uzun açıklama" name="uzunaciklama">@if(old('uzunaciklama')) {{old('uzunaciklama')}} @else {{$restoran->uzunaciklama}} @endif</textarea>
                      </div>

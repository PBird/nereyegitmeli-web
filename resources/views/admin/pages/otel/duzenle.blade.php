 ...
 @include('admin.layouts.messages')
   ...
   <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>#{{$sehir->isim}} - {{$otel->isim}}</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="{{route('sehir.index')}}">Sehirler</a></li>
              <li class="breadcrumb-item"><a href="{{route('Otel.showOteller',$sehir->id)}}">#{{$sehir->isim}} - Oteller</a></li>
              <li class="breadcrumb-item active">Otel Düzenle </li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
            ...
            <div class="card-body">
              ...
              <div class="card card-primary">
                  <div class="card-header">
                    <h3 class="card-title">{{$otel->isim}}</h3>
                  </div>
                  <!-- /.card-header -->
                  <!-- form start -->
                  <form role="form" id="olusturForm" action="{{route('otel.update',$otel->id)}}" method="POST" enctype="multipart/form-data">
                  {{method_field('put')}}
                  {{csrf_field()}}
                   <input type="hidden" id="markerAddress" name="markerAddress" value="@if(old('markerAddress')) {{old('markerAddress')}} @else {{$otel->markerAddress}} @endif">
                   <input type="hidden" id="position" name="position" value="@if(old('position')) {{old('position')}} @else {{$otel->position}} @endif">
                  <input type="hidden" name="sehir" value="{{$sehir->id}}">
                    <div class="card-body">
                      <div class="form-group">
                        <label for="isim">İsim</label>
                        <input type="text" class="form-control" id="isim" name="isim" placeholder="Otel ismini giriniz" value="@if(old('isim')) {{old('isim')}} @else {{$otel->isim}} @endif">
                      </div>
                      <div class="form-group">
                          <label>Adres</label>
                          <textarea class="form-control" rows="3" placeholder="Adresi giriniz" name="adres">@if(old('adres')) {{old('adres')}} @else {{$otel->adres}} @endif</textarea>
                      </div>
                      ...
                      <div class="input-group">
                        ...
                        <input type="text" class="form-control bfh-phone" data-format="+0 (ddd) ddd-dddd"  name="telefon" value="@if(old('phone')) {{old('phone')}} @else {{$otel->phone}} @endif">
                      </div>

                      <div class="form-group">
                        <label for="aciklama">Kısa Açıklama</label>
                        <input type="text" class="form-control" id="aciklama" name="kisaaciklama" placeholder="Kısa açıklama" value="@if(old('kisaaciklama')) {{old('kisaaciklama')}} @else {{$otel->kisaaciklama}} @endif">
                      </div>
                       <div class="form-group">
                        <label for="aciklama">Uzun Açıklama</label>
                        <textarea class="form-control" rows="3" placeholder="Uzun açıklam" name="uzunaciklama">@if(old('uzunaciklama')) {{old('uzunaciklama')}} @else {{$otel->uzunaciklama}} @endif</textarea>
                      </div>

                       <div class="form-group ">

                         <div id="home" class="tab-pane  in ">
                          <label for="exampleInputFile">Otel resmi</label>
